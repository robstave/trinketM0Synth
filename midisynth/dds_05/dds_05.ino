...
// cc for vol osc1 and vol osc 2 and detune
...
void TC5_Handler()                                         // Interrupt Service Routine (ISR) for timer TC4
{
  ...
  if (TC5->COUNT16.INTFLAG.bit.OVF && TC5->COUNT16.INTENSET.bit.OVF)
  {
    ...
    signed int v2 = ((signed char)pgm_read_byte_near(SinTable + icnt2) )* volumeOsc2;


    int value = ((v1 + v2) * 2) + 512;
...
void noteOn(byte channel, byte pitch, byte velocity) {
  ...
  if (channel == 0) {
    ...
    tword_m2 = (uint32_t)(tword_m1 * detuneOsc2);
